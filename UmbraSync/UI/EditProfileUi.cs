...
public class EditProfileUi : WindowMediatorSubscriberBase
{
    ...
    private void DrawBbCodeToolbar(ref string text)
    {
        ...
        {
            for (int i = 0; i < MoodleColors.Length; i++)
            {
                var (name, color) = MoodleColors[i];
                ImGui.PushStyleColor(ImGuiCol.Button, color);
                ImGui.PushStyleColor(ImGuiCol.ButtonHovered, color with { W = 0.7f });
                ImGui.PushStyleColor(ImGuiCol.ButtonActive, color with { W = 0.5f });
                if (ImGui.Button($"##color_{name}", new Vector2(24, 24) * ImGuiHelpers.GlobalScale))
                {
                ...
                if ((i + 1) % 5 != 0) ImGui.SameLine(0, spacing);
            }
            ImGui.EndPopup();
        }
